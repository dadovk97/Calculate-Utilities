...
class CountWater : AppCompatActivity() {
    ...
    private var waterCompanyName = ""
    private var waterPrice = 0.0
    private var saveDateYearWater = 0
    private var saveDateMonthWater = 0
    private var saveDateDayWater = 0
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btnCountWater.setOnClickListener{
            ...
            if (checkIfFirstReadingGasIsHigher()) {
                Toast.makeText(this@CountWater,
        ...
        binding.btnSaveWater.setOnClickListener{
            ...
            if(!ifPressedCountWater)
                Toast.makeText(this@CountWater, "You have to count before saving!", Toast.LENGTH_LONG).show()
            else if (!ifPressedDateWater)
                Toast.makeText(this@CountWater, "You have to select date before saving!", Toast.LENGTH_LONG).show()
            else
                saveWaterToFirebase()
        }
    ...
    private fun pickWaterCompany(){
        ...
        spinnerCompany.onItemSelectedListener = object :
            ...
            override fun onNothingSelected(p0: AdapterView<*>?) {
                Toast.makeText(this@CountWater, "You have to select water company!", Toast.LENGTH_LONG).show()
            }
        }
    }
    @SuppressLint("SetTextI18n")
    private fun countWater(){
      var waterDifference = binding.txtLastReadingWater.text.toString().toInt().toDouble() - binding.txtFirstReadingWater.text.toString().toInt().toDouble()
    ...
    private fun showDateView(){
        ...
        val datePicker = DatePickerDialog(this, { _, dateYear, dateMonth, dayOfMonth ->
            saveDateYearWater = dateYear
            saveDateMonthWater = dateMonth + 1
            saveDateDayWater = dayOfMonth },year,month,day)
        datePicker.show()
    ...
    private fun saveWaterToFirebase()
    {
        ...
        water["User"] = user
        water["Year"] = saveDateYearWater
        water["Month"] = saveDateMonthWater
        water["Day"] = saveDateDayWater
